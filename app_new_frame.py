...
if page == "1. Executive Overview":
    ...
    total_revenue, total_orders, active_customers, return_rate, aov = get_total_metrics(filtered_orders)
    ...
    c8.metric("Data Through", str(filtered_orders["order_date"].max().date()) if not filtered_orders.empty else "N/A")

    st.subheader("Performance direction")
    col1, col2 = st.columns([2, 1])
    with col1:
        st.plotly_chart(make_revenue_trend(filtered_orders), use_container_width=True)
    with col2:
        st.plotly_chart(make_category_sunburst(filtered_orders), use_container_width=True)

    st.markdown("""
    <div class="insight-box">
    <b>How to read this page:</b> The goal is not only to report revenue, but to identify where the owner should look next.
    If growth is concentrated in only a few categories or customer groups, the business may be more fragile than the top-line number suggests.
    </div>
    """, unsafe_allow_html=True)
    ...
    <ul>
        <li>Which business area should leadership investigate first?</li>
        <li>Is growth broad-based or dependent on a few categories?</li>
        <li>Should the owner prioritize revenue growth, margin improvement, traffic acquisition, or operational simplification?</li>
    </ul>
    </div>
    """, unsafe_allow_html=True)
