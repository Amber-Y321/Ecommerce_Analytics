...
# -----------------------------------------------------------------------------
    ...
    The owner considered reducing rent, but the risk is losing local customers who contribute meaningfully to revenue.
    This page frames the decision as a break-even scenario: rent savings must be large enough to offset local revenue at risk.
    </div>
    """, unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------------
    # 1) Local revenue dependence preparation
    # -------------------------------------------------------------------------
    location_col = None
    for possible_col in ["is_local", "state", "location", "customer_state"]:
    ...
    local_summary["revenue_share"] = np.where(
        local_summary["revenue"].sum() > 0,
        local_summary["revenue"] / local_summary["revenue"].sum(),
        0
    )

    st.subheader("1. Revenue dependency on local customers")
    st.markdown("""
    <div class="insight-box">
    <b>Question answered:</b> How much revenue is tied to local customers who could be affected by relocation?<br><br>
    Hover over the chart to see revenue, orders, customers, average order value, and revenue share.
    </div>
    """, unsafe_allow_html=True)

    fig_local = px.sunburst(
        local_summary,
        path=["customer_group"],
        values="revenue",
        title="Revenue Dependency by Local vs Non-local Customers",
        hover_data={
            "orders": ":,.0f",
            "customers": ":,.0f",
            "avg_order_value": ":$,.0f",
    ...
    fig_local.update_layout(margin=dict(t=60, l=0, r=0, b=0), height=500)
    ...
    st.markdown("""
    <div class="insight-box">
    <b>Question answered:</b> How much local revenue could the business afford to lose before relocation becomes unattractive?<br><br>
    ...
    local_revenue = local_summary.loc[
        local_summary["customer_group"].eq("Local / MA"), "revenue"
    ].sum()
    local_revenue_share = local_revenue / total_revenue if total_revenue else 0

    c1, c2, c3 = st.columns(3)
    c1.metric("Total Revenue", money(total_revenue))
    c2.metric("Local Revenue", money(local_revenue))
    c3.metric("Local Revenue Share", pct(local_revenue_share))
    ...
    local_revenue_loss_rate = st.slider(
        "Potential loss rate of local revenue",
        0, 100, 20, step=5
    ) / 100
    online_recapture_rate = st.slider(
        "Percent of lost local revenue recaptured online",
        0, 100, 10, step=5,
        help="Example: some local customers may continue purchasing online even if the store moves."
    ) / 100

    gross_revenue_loss = local_revenue * local_revenue_loss_rate
    ...
    c2.metric("Gross Local Revenue Loss", money(gross_revenue_loss))
    c3.metric("Online Recapture", money(recaptured_revenue))
    c4.metric("Net Effect", money(net_effect))

    scenario_chart = pd.DataFrame({
        "Component": ["Rent Savings", "Gross Local Revenue Loss", "Online Recapture", "Net Effect"],
    ...
    fig_scenario = px.bar(
        ...
        y="Value",
        text="Value",
        title="Scenario View: Rent Savings vs. Local Revenue Risk",
        template="plotly_white"
    )
    ...
    # -------------------------------------------------------------------------
    if net_effect > 50000:
        scenario_message = "Under the current assumptions, relocation appears financially favorable, but the owner should still validate local customer behavior and brand visibility risk before moving."
    elif net_effect > 0:
        scenario_message = "Under the current assumptions, relocation is only marginally favorable. The owner should negotiate rent, test local customer retention, or strengthen online recapture before making the move."
    else:
        scenario_message = "Under the current assumptions, rent savings may not fully offset local revenue risk. The owner should be cautious about relocating without stronger evidence that local revenue can be retained or recaptured online."
