...
if page == "1. Executive Overview":
    ...
    # Whole-business metrics should stay unfiltered so Page 1 remains a true executive overview.
    overview_orders = orders_df.copy()
    ...
    Filters in this section apply only to the three trend charts below. The rest of the overview remains whole-business level so concentration and alert signals are not accidentally hidden.
    </div>
    """, unsafe_allow_html=True)

    trend_orders = orders_df.copy()
    f1, f2, f3 = st.columns([1.1, 1.4, 1.1])
    with f1:
    ...
    with f3:
        t_min_date = trend_orders["order_date"].min().date() if not trend_orders.empty else date(2019, 1, 1)
        t_max_date = trend_orders["order_date"].max().date() if not trend_orders.empty else date(2024, 12, 31)
        trend_date_range = st.date_input("Date range", [t_min_date, t_max_date], key="page1_trend_date_range")
    if len(trend_date_range) == 2:
        trend_start, trend_end = trend_date_range
        trend_orders = trend_orders[
            (trend_orders["order_date"] >= pd.to_datetime(trend_start)) &
            (trend_orders["order_date"] <= pd.to_datetime(trend_end))
        ].copy()
