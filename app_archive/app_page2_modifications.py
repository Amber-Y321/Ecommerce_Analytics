...
# -----------------------------------------------------------------------------
    ...
    This page evaluates revenue concentration at both category and subcategory levels, then translates the findings into an assortment strategy discussion.
    The goal is not to force a yes/no answer, but to help the owner decide whether to keep a broad assortment or gradually pivot toward a more focused mix.
    </div>
    """, unsafe_allow_html=True)
    ...
    if page2_orders.empty:
        ...
        # ---------------------------------------------------------------------
        # 1. Revenue mix: category + subcategory
        # ---------------------------------------------------------------------
        st.subheader("1. Revenue mix: category and subcategory breakdown")
        st.caption(
            "This section answers: Which product areas actually drive revenue, and is revenue concentrated in a few product types?"
        )
        ...
        total_rev_page2 = cat_subcat_rev["revenue"].sum()
        cat_subcat_rev["revenue_share"] = np.where(
            total_rev_page2 > 0,
            cat_subcat_rev["revenue"] / total_rev_page2,
            0
        )

        # Add gross margin and return rate where fields are available.
        if "cost" in page2_orders.columns:
            gp = (
                page2_orders
                .groupby(["category", "subcategory"])
                .apply(lambda x: (x["price"] - x["cost"]).sum())
                .reset_index(name="gross_profit")
            )
            cat_subcat_rev = cat_subcat_rev.merge(gp, on=["category", "subcategory"], how="left")
            cat_subcat_rev["gross_margin"] = np.where(
                cat_subcat_rev["revenue"] > 0,
                cat_subcat_rev["gross_profit"] / cat_subcat_rev["revenue"],
                0
            )

        if "status" in page2_orders.columns:
            ret = page2_orders.assign(is_returned=page2_orders["status"].eq("Returned"))
            ret_summary = (
                ret.groupby(["category", "subcategory"])["is_returned"]
                .mean()
                .reset_index(name="return_rate")
            )
            cat_subcat_rev = cat_subcat_rev.merge(ret_summary, on=["category", "subcategory"], how="left")
        ...
        with col1:
            ...
            category_summary["revenue_share"] = np.where(
                category_summary["revenue"].sum() > 0,
            ...
            fig_cat_bar = px.bar(
                ...
                labels={"category": "Category", "revenue": "Revenue", "revenue_share": "Revenue Share"},
                template="plotly_white"
            )
            fig_cat_bar.update_traces(texttemplate="%{text:.1%}", textposition="outside")
            fig_cat_bar.update_layout(yaxis_tickprefix="$", margin=dict(t=60, l=20, r=20, b=40))
        ...
        with col2:
            ...
            top_subcat_summary["revenue_share"] = np.where(
                total_rev_page2 > 0,
                top_subcat_summary["revenue"] / total_rev_page2,
                0
            )

            fig_subcat_bar = px.bar(
                top_subcat_summary.sort_values("revenue", ascending=True),
                x="revenue",
                y="subcategory",
                orientation="h",
                text="revenue_share",
                title="Top Subcategories by Revenue",
                labels={"subcategory": "Subcategory", "revenue": "Revenue", "revenue_share": "Revenue Share"},
                template="plotly_white"
            )
            fig_subcat_bar.update_traces(texttemplate="%{text:.1%}", textposition="outside")
            fig_subcat_bar.update_layout(xaxis_tickprefix="$", margin=dict(t=60, l=20, r=20, b=40))
            st.plotly_chart(fig_subcat_bar, use_container_width=True)

        st.markdown("""
        <div class="insight-box">
        <b>How to read this:</b> The sunburst shows whether revenue is broadly distributed or concentrated in a few product families.
        If a small number of subcategories carry a large share of revenue, the owner can simplify merchandising decisions around those categories instead of treating every product line equally.
        </div>
        """, unsafe_allow_html=True)

        # ---------------------------------------------------------------------
        # 2. Revenue growth over time: category and subcategory
        # ---------------------------------------------------------------------
        st.subheader("2. Growth pattern over time")
        st.caption(
            "This section answers: Are the current revenue drivers stable, seasonal, or changing over time?"
        )

        trend_df = page2_orders.copy()
        trend_df["order_month"] = trend_df["order_date"].dt.to_period("M").dt.to_timestamp()

        tab_cat, tab_subcat = st.tabs(["Category trend", "Subcategory trend"])
        ...
        with tab_subcat:
            ...
            subcategory_trend = (
                trend_df[trend_df["subcategory"].isin(top_subcats)]
            ...
            fig_subcategory_trend.update_layout(
                yaxis_tickprefix="$",
                hovermode="x unified",
                margin=dict(t=60, l=20, r=20, b=40)
            )
        ...
        # ---------------------------------------------------------------------
        # 3. Assortment strategy: broad assortment vs. focused growth mix
        # ---------------------------------------------------------------------
        st.subheader("3. Assortment strategy: broad assortment vs. focused growth mix")
        st.caption(
            "This section answers: If the owner wants to simplify operations, which middle-ground assortment could preserve revenue while reducing complexity?"
        )

        apparel_subcats = ["Cardigan", "Pants", "Shirts & Tops", "Coats & Jackets", "Skirts"]
        strategic_focus_subcats = ["Handbags", "Bracelets", "Charms & Pendants", "Scarf", "Cardigan"]

        assortment = page2_orders.copy()
        assortment["assortment_group"] = np.select(
            [
                assortment["subcategory"].isin(strategic_focus_subcats),
                assortment["subcategory"].isin(apparel_subcats),
            ],
            [
                "Focused Mix: Handbags / Jewelry / Scarves / Cardigan",
                "Other Apparel",
            ],
        ...
        assortment_summary = (
            assortment
            .groupby("assortment_group", as_index=False)
            .agg(
                revenue=("price", "sum"),
                orders=("order_id", "nunique"),
                customers=("customer_id", "nunique"),
        ...
        assortment_summary["revenue_share"] = np.where(
            assortment_summary["revenue"].sum() > 0,
            assortment_summary["revenue"] / assortment_summary["revenue"].sum(),
            0
        )
        ...
        if "cost" in assortment.columns:
            gp_assort = (
                assortment
                .groupby("assortment_group")
                .apply(lambda x: (x["price"] - x["cost"]).sum())
                .reset_index(name="gross_profit")
            )
            assortment_summary = assortment_summary.merge(gp_assort, on="assortment_group", how="left")
            assortment_summary["gross_margin"] = np.where(
                assortment_summary["revenue"] > 0,
                assortment_summary["gross_profit"] / assortment_summary["revenue"],
                0
            )

        if "status" in assortment.columns:
            ret_assort = assortment.assign(is_returned=assortment["status"].eq("Returned"))
            ret_assort_summary = (
                ret_assort.groupby("assortment_group")["is_returned"]
                .mean()
                .reset_index(name="return_rate")
            )
            assortment_summary = assortment_summary.merge(ret_assort_summary, on="assortment_group", how="left")
        ...
        with col1:
            fig_assort = px.bar(
                ...
                y="revenue",
                text="revenue_share",
                title="Revenue Trade-Off by Assortment Strategy",
                labels={
                    "assortment_group": "Assortment Strategy",
                    "revenue": "Revenue",
        ...
        with col2:
            display_cols = ["assortment_group", "revenue", "revenue_share", "orders", "customers", "product_count", "avg_price"]
            if "gross_margin" in assortment_summary.columns:
                display_cols.append("gross_margin")
            if "return_rate" in assortment_summary.columns:
                display_cols.append("return_rate")

            st.dataframe(
                assortment_summary[display_cols].style.format({
                    "revenue": "${:,.0f}",
                    "revenue_share": "{:.1%}",
                    "avg_price": "${:,.0f}",
                    "gross_margin": "{:.1%}",
        ...
        # Timing matters: show whether the focused mix is becoming stronger or weaker.
        timing = assortment.copy()
        timing["quarter"] = timing["order_date"].dt.to_period("Q").astype(str)
        timing_summary = (
            timing
            .groupby(["quarter", "assortment_group"], as_index=False)
            .agg(revenue=("price", "sum"))
        )
        ...
        fig_timing = px.line(
            ...
            x="quarter",
            y="revenue_share",
            color="assortment_group",
            markers=True,
            title="Assortment Revenue Share Over Time",
            labels={
                "quarter": "Quarter",
                "revenue_share": "Revenue Share",
                "assortment_group": "Assortment Strategy"
        ...
        st.markdown("""
        <div class="decision-box">
        <b>Decision supported:</b> The assortment decision does not need to be all-or-nothing.
        A middle-ground strategy may focus on jewelry-style accessories, handbags, scarves, and selected cardigans instead of immediately exiting apparel entirely.<br><br>
        <b>Why timing matters:</b> If the focused mix is gaining share over recent quarters, the owner can gradually pivot inventory and marketing toward those categories.
        If the focused mix is seasonal or weakening, a slower test-and-learn approach is safer.
        </div>
        """, unsafe_allow_html=True)

        st.markdown("""
        <div class="insight-box">
        <b>Analyst framing:</b> The dashboard helps the owner compare revenue preservation against operational simplification.
        The recommendation should be treated as a staged decision: test the focused mix first, monitor revenue share and return rate, then decide whether to reduce broader apparel investment.
        </div>
        """, unsafe_allow_html=True)
