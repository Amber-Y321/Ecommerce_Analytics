...
# -----------------------------------------------------------------------------
    ...
    This page frames the decision as a break-even scenario: rent savings must be large enough to offset local/walk-in revenue at risk.
    </div>
    """, unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------------
    # 1) Locality + shopping method preparation
    # -------------------------------------------------------------------------
    location_col = None
    for possible_col in ["is_local", "state", "location", "customer_state"]:
    ...
    if location_col == "is_local":
        scenario_df["customer_group"] = np.where(
            scenario_df["is_local"].astype(bool),
            "Local / MA",
            "Non-local"
        )
    elif location_col in ["state", "customer_state", "location"]:
        scenario_df["customer_group"] = np.where(
            scenario_df[location_col].astype(str).str.upper().str.contains("MA|MASSACHUSETTS", na=False),
            "Local / MA",
            "Non-local"
        )
    else:
    ...
    # Try to use a real shopping-method field if it exists.
    # If not available, keep the field as Unknown rather than inventing behavior.
    shopping_method_col = None
    for possible_col in [
        "shopping_method", "purchase_method", "order_method", "order_channel",
        "sales_channel", "channel", "shopping_channel"
    ]:
        if possible_col in scenario_df.columns:
            shopping_method_col = possible_col
            break

    if shopping_method_col:
        scenario_df["shopping_method"] = scenario_df[shopping_method_col].astype(str).str.title()
    else:
        scenario_df["shopping_method"] = "Unknown"
        st.info(
            "Shopping method was not found in the orders data. "
            "If you want online vs walk-in analysis, add a column such as `shopping_method` with values like `Online` and `Walk-in`. "
            "For now, the relocation scenario uses local customer revenue as the main risk proxy."
        )

    local_method_summary = scenario_df.groupby(["customer_group", "shopping_method"]).agg(
        revenue=("price", "sum"),
        orders=("order_id", "nunique"),
        customers=("customer_id", "nunique"),
        avg_order_value=("price", "mean")
    ).reset_index()
    local_method_summary["revenue_share"] = np.where(
        local_method_summary["revenue"].sum() > 0,
        local_method_summary["revenue"] / local_method_summary["revenue"].sum(),
        0
    )

    st.subheader("1. Local and shopping-method revenue dependence")
    st.markdown("""
    <div class="insight-box">
    <b>Question answered:</b> How much revenue is tied to local customers and shopping behavior that could be affected by relocation?<br><br>
    Hover over the chart to see revenue, orders, customers, average order value, and revenue share.
    </div>
    """, unsafe_allow_html=True)

    # Sunburst works better than a simple pie because it can show customer location
    # and shopping method together while keeping hover details in the chart.
    fig_local_method = px.sunburst(
        local_method_summary,
        path=["customer_group", "shopping_method"],
        values="revenue",
        title="Revenue Dependence by Locality and Shopping Method",
        hover_data={
            "orders": ":,.0f",
            "customers": ":,.0f",
            "avg_order_value": ":$,.0f",
        ...
        labels={
            ...
            "shopping_method": "Shopping Method",
            "revenue": "Revenue",
            "orders": "Orders",
            "customers": "Customers",
            "avg_order_value": "AOV",
    ...
    fig_local_method.update_traces(
        hovertemplate=(
            "<b>%{label}</b><br>"
            "Revenue: $%{value:,.0f}<br>"
            ...
            "AOV: $%{customdata[2]:,.0f}<extra></extra>"
        )
    )
    fig_local_method.update_layout(margin=dict(t=60, l=0, r=0, b=0), height=520)
    st.plotly_chart(fig_local_method, use_container_width=True)

    # -------------------------------------------------------------------------
    # 2) Relocation scenario model
    # -------------------------------------------------------------------------
    st.subheader("2. Rent-saving break-even scenario")
    st.markdown("""
    <div class="insight-box">
    <b>Question answered:</b> How much local/walk-in revenue could the business afford to lose before relocation becomes unattractive?<br><br>
    The calculation should be scenario-based, not AI-estimated. AI can help explain the trade-off, but the financial estimate should come from transparent assumptions that the owner can adjust.
    </div>
    """, unsafe_allow_html=True)

    total_revenue, _, _, _, _ = get_total_metrics(scenario_df)
    local_revenue = local_method_summary.loc[
        local_method_summary["customer_group"].eq("Local / MA"), "revenue"
    ].sum()

    # If shopping method exists, walk-in local revenue is the most relocation-sensitive pool.
    # If not, use total local revenue as a conservative proxy.
    if shopping_method_col:
        walkin_mask = (
            local_method_summary["customer_group"].eq("Local / MA")
            & local_method_summary["shopping_method"].str.lower().str.contains("walk|store|in-person|offline", na=False)
        )
        local_walkin_revenue = local_method_summary.loc[walkin_mask, "revenue"].sum()
        revenue_at_risk_base = local_walkin_revenue if local_walkin_revenue > 0 else local_revenue
        risk_label = "Local walk-in revenue at risk" if local_walkin_revenue > 0 else "Local revenue at risk"
    else:
        revenue_at_risk_base = local_revenue
        risk_label = "Local revenue at risk"

    c1, c2, c3 = st.columns(3)
    c1.metric("Total Revenue", money(total_revenue))
    c2.metric("Local Revenue", money(local_revenue))
    c3.metric(risk_label, money(revenue_at_risk_base))

    annual_rent_savings = st.slider(
        "Estimated annual rent savings from relocation",
        0, 300000, 60000, step=5000
    )
    local_revenue_loss_rate = st.slider(
        "Potential loss rate of local / walk-in revenue",
        0, 100, 20, step=5
    ) / 100
    online_recapture_rate = st.slider(
        "Percent of lost revenue recaptured online",
        0, 100, 10, step=5,
        help="Example: some customers may continue purchasing online even if the store moves."
    ) / 100

    gross_revenue_loss = revenue_at_risk_base * local_revenue_loss_rate
    recaptured_revenue = gross_revenue_loss * online_recapture_rate
    estimated_net_revenue_loss = gross_revenue_loss - recaptured_revenue
    net_effect = annual_rent_savings - estimated_net_revenue_loss

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Rent Savings", money(annual_rent_savings))
    c2.metric("Gross Revenue Loss", money(gross_revenue_loss))
    c3.metric("Online Recapture", money(recaptured_revenue))
    c4.metric("Net Effect", money(net_effect))

    scenario_chart = pd.DataFrame({
        "Component": ["Rent Savings", "Gross Revenue Loss", "Online Recapture", "Net Effect"],
        "Value": [annual_rent_savings, -gross_revenue_loss, recaptured_revenue, net_effect]
    })
    fig_scenario = px.bar(
        scenario_chart,
        x="Component",
    ...
    fig_scenario.update_layout(margin=dict(l=20, r=20, t=60, b=35), yaxis_title="Estimated Annual Impact")
    st.plotly_chart(fig_scenario, use_container_width=True)
    ...
    # -------------------------------------------------------------------------
    if net_effect > 0:
        scenario_message = "Under the current assumptions, relocation could be financially attractive, but the owner should still consider brand visibility and customer experience risk."
    else:
        scenario_message = "Under the current assumptions, rent savings may not fully offset local/walk-in revenue risk. The owner should negotiate rent, improve online recapture, or test demand before moving."

    st.markdown(f"""
    <div class="decision-box">
    <b>Decision supported:</b> The dashboard does not decide relocation automatically. It provides a transparent break-even framework.<br><br>
    <b>Current scenario readout:</b> {scenario_message}<br><br>
    <b>How AI should be used here:</b> AI should not estimate the revenue loss by itself. The loss estimate should come from owner-adjustable assumptions, while AI can optionally help summarize risks, explain trade-offs, and draft next-step recommendations.
    </div>
    """, unsafe_allow_html=True)
